...
// The exact banner text stamped into every file this generator writes (see renderCursorAgent
// below). The stale-file prune step in cmdGenerate treats this marker as the sole proof that a
// given destDir/*.md file was produced by this tool — anything lacking it is left untouched,
// even if its name has no matching source file, so a Cursor operator's own hand-authored
// subagent in the same directory is never at risk of being silently deleted.
const GENERATED_FILE_MARKER = 'GENERATED FILE — DO NOT HAND-EDIT';
...
function cmdGenerate(opts) {
  ...
  // SAFETY: .cursor/agents/ is Cursor's real, general-purpose subagent directory — an operator
  // may legitimately keep their own hand-authored subagent files there. Only ever delete a file
  // that carries this generator's own GENERATED_FILE_MARKER banner (written by
  // renderCursorAgent above); anything else is left untouched with a warning, never silently
  // deleted and never silently ignored.
  ...
  for (const stale of existing) {
    ...
    if (!staleContent.includes(GENERATED_FILE_MARKER)) {
      process.stderr.write(
        `gmj-cursor-adapter: warning: leaving non-generated file untouched (no matching source, but missing "${GENERATED_FILE_MARKER}" marker): ${stalePath}\n`
      );
      continue; // never delete a file this tool didn't generate
    }
    fs.rmSync(stalePath);
    process.stdout.write(`gmj-cursor-adapter: removed stale ${stalePath} (no matching source file)\n`);
  }
