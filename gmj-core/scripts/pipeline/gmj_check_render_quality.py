...
governs both the clipped-content bbox-vs-cropbox comparison and the block-overlap-area
tolerance for the empty/overlapping check (a single shared, documented tolerance knob rather
...
# Pinned, tunable default tolerance (points) shared by the clipped-content bbox-vs-cropbox
# check and the block-overlap-area check for empty/overlapping regions. 3.0pt comfortably
# exceeds normal sub-pixel rendering jitter while still catching a genuine clipping bug.
...
# Map a candidate.yaml TOP_LEVEL key to its expected labels.yaml label key(s). Some
# candidate.yaml keys are rendered under a differently-named label key (e.g.
# professional_experience -> "experience") -- this is the single place that records that.
_LABEL_KEY_FOR = {
    "summary": "summary",
    "expertise": "expertise",
...
def find_missing_sections(
    ...
    ``labels`` (a ``TOP_LEVEL`` key with no matching label entry, e.g. ``name``/``photo``/
    ``title``/``contact``, is skipped -- it has no section heading of its own). Both the
    expected label and ``rendered_text`` are normalized (NFC + casefold + whitespace
    collapse) before the substring comparison, so ua/ru Cyrillic round-trips correctly.
    """
    normalized_text = _normalize(rendered_text)
    defects: list[dict] = []
    for key, label in _expected_labels(candidate, labels):
...
def find_empty_and_overlapping(
    doc: "fitz.Document", tolerance: float = DEFAULT_CLIP_TOLERANCE,
) -> list[dict]:
...
def run_checks(
    ...
    a genuine script-execution failure (bad YAML, unopenable PDF)."""
    try:
    ...
    try:
        ...
        defects: list[dict] = []
        defects.extend(find_missing_sections(candidate, labels, full_text))
        defects.extend(find_clipped_content(doc, tolerance))
        defects.extend(find_empty_and_overlapping(doc, tolerance))
        return defects, None
    finally:
        doc.close()
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help=f"Tolerance in points for bbox-vs-cropbox / overlap checks (default: {DEFAULT_CLIP_TOLERANCE})",
    )
