...
2. Drive the deterministic control plane via **`Bash`** for every safety decision — the hub never judges a gate, a cap, or delivery. The runtime loop is documented in `docs/ARCHITECTURE.md` §5.1:
   - `init_run` — freeze `execution_mode` + `retry_cap` + `run_id` into `.pipeline/runs/<run_id>/state.json` (`scripts/pipeline/state_write.py`).
   - loop — `route.py` → next step; `check_offer.py` before each dispatch; `Task(spoke)`; on a gate node run `check_truth.py` / `score_fit.py` (exit 0/1, no bypass) → `record_gate.py`; on FAIL `record_retry.py --increment` → `check_cap.py` (below-cap → `map_feedback.py` → `Task(artifact-composer)`; at-cap → HARD STOP naming the failing artifact + reason).
   - deliver — `check_delivery.py` (Gate A ∧ Gate B recorded pass) before any artifact is delivered.
...
- **`run_id`** — optional; a fresh id is generated if absent. Scopes `.pipeline/runs/<run_id>/` (state + gate logs). Passing an existing `run_id` **resumes** that run via `route.py`.
