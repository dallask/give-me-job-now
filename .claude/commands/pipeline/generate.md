...
description: Run the cv-generator spoke to render a gate-passed artifact to PDF via scripts/cv/render_cv.py (live E2E is Phase 8).
---
...
- **Spoke:** `Task(subagent_type: cv-generator)` — render-only, renders the gate-passed artifact:
  ```bash
  python3 scripts/cv/render_cv.py [--lang ua|ru]   # → output/cv/*.pdf
  ```
