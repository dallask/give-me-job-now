...
description: Run the offer-scout spoke (board-search or single-offer), scoped by config/sources.yaml, then hand to /pipeline/freeze.
---

## What this step names (thin wrapper — no control logic here)

- **Spoke:** `Task(subagent_type: offer-scout)` — board-search (rank N offers) or single-offer intake.
  `offer-scout` **must** read `config/sources.yaml` (the board/geo/language allow-list) before any web search; it may never search outside that scope.
