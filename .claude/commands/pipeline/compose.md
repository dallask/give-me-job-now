...
description: Run the artifact-composer spoke for one artifact type (CV | cover_letter | interview_prep); names record_retry.py for the per-type counter.
---

## What this step names (thin wrapper — no control logic here)

- **Spoke:** `Task(subagent_type: artifact-composer)` for **one** artifact type (`cv` | `cover_letter` | `interview_prep`), reading canonical `config/candidate.yaml` (read-only) + the frozen `offer-spec.json`. On a retry it also receives ONLY the structured `map_feedback.py` output (`{missing_must_haves, fabricated_claims, gate}`) — never gate prose or a transcript.
- **Per-(offer, type) retry counter** (deterministic):
  ```bash
  python3 scripts/artifacts/record_retry.py \
