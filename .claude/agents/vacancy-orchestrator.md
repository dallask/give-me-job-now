...
description: Hub orchestrator for the job/CV collective. Use when the user wants end-to-end vacancy research, candidate sourcing analysis, config updates, CV PDF generation, review, or enhancement. Invokes the routing schema User Request → Routing Analysis → Agent Selection → Task Delegation → Quality Gate → Result. Only this role should spawn subagents via Task.
...
4. **Quality gate**: After deliverable steps (`candidate-configurator`, `cv-generator`, `cv-enhancer`), use `Task` to run **`cv-deliverable-gate`** with the acceptance criteria from the router or prior step.
