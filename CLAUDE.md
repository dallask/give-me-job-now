...
- `/gmj-dashboard` — live btop-style pipeline cockpit (read-only by default; opt-in `--manage`).

**Artifact depth:** three artifact types — CV, cover letter, interview-prep — generated by
default per single-offer run (narrow via `--artifact-types=cv,cover_letter`), each rendered by
Python (`scripts/cv/gmj_render_cv.py`, `gmj_render_cover_letter.py`, `gmj_render_interview_prep.py`).
CV render always attempts a first-class `.html` sibling alongside the PDF. Run state lives
per-run under `.pipeline/runs/<run_id>/` (git-ignored).
...
- `scripts/` — Python spokes + tooling (`cv/`, `offers/`, `artifacts/`, `contracts/`, `pipeline/`,
  `preferences/`, `dashboard/` for the `gmj-dashboard` cockpit, `runtime/` for the EXPERIMENTAL
  Claude Agent SDK runtime prototype).
...
- `gmj-core/` — packaged standalone payload + installers: `gmj-core/bin/install.sh` (one-script
  fresh-machine setup) and `gmj-core/bin/gmj-tools.cjs` (payload staging) for clean installs;
  also hosts the EXPERIMENTAL `gmj-core/bin/gmj-cursor-adapter.cjs` Cursor-roster generator.
