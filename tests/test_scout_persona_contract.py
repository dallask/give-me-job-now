...
green-gate. Asserts over ``.claude/agents/offer-scout.md`` and
``.claude/agents/vacancy-orchestrator.md``:

- offer-scout stays a NON-SPAWNING worker ("Does not spawn subagents" + the Rules line
  forbidding ``Task``) — T-11-08,
- offer-scout board search uses JOB-SEEKER framing (no plural recruiter token, no
  ``found <n> candidates`` phrasing) — SCOUT-01,
- the hub fans out ONE offer-scout Task PER BOARD in a single turn — SCOUT-03,
...
OFFER_SCOUT = REPO_ROOT / ".claude" / "agents" / "offer-scout.md"
ORCHESTRATOR = REPO_ROOT / ".claude" / "agents" / "vacancy-orchestrator.md"
...
def test_offer_scout_non_spawning() -> None:
    ...
    assert "Does not spawn subagents" in t, (
        "offer-scout.md must keep the front-matter 'Does not spawn subagents' invariant "
        "(non-spawning worker; hub owns fan-out) — T-11-08"
    )
    assert "Do **not** call `Task`" in t, (
        "offer-scout.md must keep the Rules line 'Do **not** call `Task`.' — T-11-08"
    )
...
def test_offer_scout_jobseeker_framing() -> None:
    ...
    assert RECRUITER_TOKEN not in t.lower(), (
        f"offer-scout.md must not contain the plural recruiter token '{RECRUITER_TOKEN}' "
        "(job-seeker framing, SCOUT-01)"
    )
    assert not RECRUITER_PHRASE.search(t), (
        "offer-scout.md must not use recruiter phrasing 'found <n> candidates' (SCOUT-01)"
    )
...
def test_hub_fans_out_per_board() -> None:
    ...
    assert "per board" in tl, (
        "vacancy-orchestrator.md must document per-board fan-out ('per board') — SCOUT-03"
    )
    assert "offer-scout" in tl, (
        "vacancy-orchestrator.md must name offer-scout as the per-board worker — SCOUT-03"
    )
...
def test_hub_invokes_merge() -> None:
    ...
    assert "gmj_merge_shortlists.py" in t, (
        "vacancy-orchestrator.md must invoke gmj_merge_shortlists.py as the deterministic "
...
def test_hub_keeps_single_task_holder() -> None:
    ...
    assert "only you" in tl and "call `task`" in tl, (
        "vacancy-orchestrator.md must keep 'Only you call `Task`' single-holder invariant — T-11-08"
    )
    assert "spokes never spawn spokes" in tl or "never spawn other spokes" in tl, (
        "vacancy-orchestrator.md must keep the spokes-never-spawn-spokes invariant — T-11-08"
    )
